...
AddEventHandler("OnPluginStart", function(event)
	...
	if not db:IsConnected() then return EventResult.Continue end
   	...
   	db:Query("CREATE TABLE `"..config:Fetch("admins.tablenames.admins").."` ( `steamid` varchar(128) NOT NULL, `username` varchar(128) NOT NULL, `group` text DEFAULT NULL, `flags` text DEFAULT NULL, `immunity` int(11) NOT NULL DEFAULT 0, `serverid` int(11) NOT NULL DEFAULT 1 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;", function(err, result)
       	if #result > 0 then
			db:Query("ALTER TABLE `"..config:Fetch("admins.tablenames.admins").."` ADD UNIQUE KEY `steamid` (`steamid`,`username`);")
		end
	end)

	db:Query("CREATE TABLE `"..config:Fetch("admins.tablenames.groups").."` ( `groupname` varchar(128) NOT NULL, `group_displayname` text NOT NULL, `flags` text NOT NULL, `serverid` int(11) NOT NULL DEFAULT 1 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;", function(err, result)
       	if #result > 0 then
			db:Query("ALTER TABLE `"..config:Fetch("admins.tablenames.groups").."` ADD UNIQUE KEY `groupname` (`groupname`);")
		end
	end)
...
function GroupsLoader(cb)
	...
	db:Query("select * from `"..config:Fetch("admins.tablenames.groups").."` where serverid = "..config:Fetch("admins.serverid"), function(err, result)
...
function LoadAdmins(cb)
	db:Query("select * from `"..config:Fetch("admins.tablenames.admins").."` where serverid = "..config:Fetch("admins.serverid"), function(err, result)
		if #err > 0 then return print("ERROR: "..err) end
		for i=1,#result do
